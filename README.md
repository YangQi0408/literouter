...
`literouter` 是一个兼顾个人自用与 API 分发的高性能 AI 网关。它对外提供统一端口（默认 `http://127.0.0.1:8787`），对内将请求按你设定的策略分发至多个上游模型中转站与官方 API。
...
支持个人自用与多客户端分发：客户端多密钥、模型/中转站组权限、持久化配额与限流共用同一套网关。详见[API 分发](docs/zh/distribution.md)。
