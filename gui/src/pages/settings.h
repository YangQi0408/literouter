...
inline void composeSettings(eui::Ui& ui, float x, float y, float width, float height) {
    ...
    components::scrollView(ui, "settings.scroll")
        ...
        .content([&](eui::Ui& contentUi, float contentWidth, float) {
            ...
            constexpr float kBreakerHeight = 156.0f + 34.0f + kCardBottomPadding;
            constexpr float kLoggingHeight = 204.0f + 32.0f + kCardBottomPadding;

            const float listenerX = leftX;
            const float listenerY = cardsTop;

            const float displayX = leftX;
            const float displayY = listenerY + kListenerHeight + columnGap;

            const float breakerX = twoColumns ? (leftX + columnWidth + columnGap) : leftX;
            const float breakerY = twoColumns ? cardsTop : (displayY + kDisplayHeight + columnGap);

            const float loggingX = breakerX;
            const float loggingY = breakerY + kBreakerHeight + columnGap;

            const float howY = twoColumns
                ? (std::max(displayY + kDisplayHeight, loggingY + kLoggingHeight) + columnGap)
                : (loggingY + kLoggingHeight + columnGap);
            const float howWidth = contentWidth - 2.0f * layout::pagePadding;
            constexpr float kHowHeight = 152.0f + 34.0f + kCardBottomPadding;
            const float howHeight = kHowHeight;

            const float canvasHeight = howY + howHeight + layout::pagePadding + 16.0f;
            ...
            contentUi.stack("settings.canvas")
                ...
                .content([&] {
                    ...
                    fieldLabel(contentUi, "settings.threshold.label", breakerX + 18.0f, breakerY + 76.0f, fieldWidth,
                    ...
                    contentUi.stack("settings.threshold.wrap")
                        ...
                        .build();
                    fieldLabel(contentUi, "settings.cooldown.label", breakerX + 18.0f, breakerY + 136.0f, fieldWidth,
                               std::string(literouter::i18n::tr("Cooldown (seconds)")));
                    contentUi.stack("settings.cooldown.wrap")
                        .position(breakerX + 18.0f, breakerY + 156.0f)
                        .size(fieldWidth, 34.0f)
                        .content([&] {
                    ...
                    fieldLabel(contentUi, "settings.capacity.label", loggingX + 18.0f, loggingY + 76.0f, fieldWidth,
                    ...
                    contentUi.stack("settings.capacity.wrap")
                        ...
                        .build();
                    fieldLabel(contentUi, "settings.bodylimit.label", loggingX + 18.0f, loggingY + 140.0f, fieldWidth,
                               std::string(literouter::i18n::tr("Retained body bytes per entry")));
                    contentUi.stack("settings.bodylimit.wrap")
                        .position(loggingX + 18.0f, loggingY + 160.0f)
                        .size(fieldWidth, 34.0f)
                        .content([&] {
                    ...
                    contentUi.stack("settings.bodies.wrap")
                        .position(loggingX + 18.0f, loggingY + 204.0f)
                        .size(fieldWidth, 32.0f)
                        .content([&] {
                    ...
                    // "How to use it"
                    detail::settingsCard(contentUi, "settings.how", leftX, howY, howWidth, howHeight,
                    ...
                    contentUi.text("settings.how.base.label")
                        .position(leftX + 18.0f, howY + 76.0f)
                        .size(140.0f, 18.0f)
                    ...
                    contentUi.text("settings.how.base")
                        .position(leftX + 160.0f, howY + 74.0f)
                        .size(howWidth - 180.0f, 20.0f)
                    ...
                    const std::string curl =
                        ...
                        "/chat/completions -H \"Content-Type: application/json\" -H \"Authorization: Bearer " +
                        keyText + "\" -d '{\"model\":\"gpt-4o\",\"messages\":[{\"role\":\"user\",\"content\":\"hi\"}]}'";
                    contentUi.rect("settings.how.curl.bg")
                        .position(leftX + 18.0f, howY + 104.0f)
                        .size(howWidth - 36.0f, 40.0f)
                        .radius(p.radiusControl)
                        .color(p.surfaceSunken)
                        .border(1.0f, withAlpha(p.border, 0.7f))
                        .build();
                    contentUi.text("settings.how.curl")
                        .position(leftX + 28.0f, howY + 104.0f)
                        .size(howWidth - 56.0f, 40.0f)
                        .text(curl)
                        .fontSize(11.5f)
                        .lineHeight(16.0f)
                        .color(p.textMuted)
                    ...
                    contentUi.text("settings.how.tip")
                        .position(leftX + 18.0f, howY + 152.0f)
                        .size(howWidth - 36.0f, 34.0f)
                        .text(tip)
                        .fontSize(11.5f)
                        .lineHeight(17.0f)
                        .wrap(true)
