...
自用默认继续使用 `http://127.0.0.1:8787`。分发部署时，将 `server.tls_cert_file` 设置为服务端证书链的绝对路径（叶证书在前，随后是中间证书），并将 `server.tls_key_file` 设置为对应的未加密 PEM 私钥绝对路径。证书的 SAN 必须覆盖客户端实际连接的域名或 IP。证书缺失、过期、尚未生效或私钥不匹配时拒绝启动，`serve --force` 也不会绕过 TLS 检查。
