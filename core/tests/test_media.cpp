...
public:
    ...
    std::atomic<bool> streaming_entered{false}, release_stream{false};
    std::mutex mutex;
    Captured captured;
    Reply reply;
        ...
        server.Post(R"(/api/v1/(audio|images)/.*)",
            ...
            if (current.chunked) {
                res.set_chunked_content_provider(current.type,
                    [this, current](std::size_t offset, httplib::DataSink &sink) {
                        const auto split = current.body.size() / 2;
                        if (offset == 0) return sink.write(current.body.data(), split);
                        if (current.hold) {
                            streaming_entered = true;
                            const auto deadline = std::chrono::steady_clock::now() + std::chrono::seconds(5);
                            while (!release_stream && std::chrono::steady_clock::now() < deadline)
...
int main() {
    ...
    LR_GROUP("existing OpenAI protocol aliases keep media on the raw wire path");
    for (const char *alias : {"openai_compatible", "openai_chat"}) {
        cfg.providers[0].protocol = alias;
        proxy.updateConfig(cfg);
        proxy.resetStats(); first.set(); second.set();
        auto aliased = client.Post("/v1/images/generations", R"({"model":"media","prompt":"hill"})", "application/json");
        LR_CHECK(aliased && aliased->status == 200);
        const auto sent = json::parse(first.last().body);
        LR_CHECK_EQ(sent["model"].get<std::string>(), "upstream-a");
        LR_CHECK_EQ(sent["prompt"].get<std::string>(), "hill");
        LR_CHECK(!sent.contains("messages") && !sent.contains("stream"));
    }

    LR_GROUP("media admission restricts client models and channels and shares rate limits across keys");
    cfg = configuration(first, second);
    cfg.providers[0].groups = {"private"};
    cfg.providers[1].groups = {"paid"};
    ClientConfig account;
    account.id = "media-client";
    account.models = {"media"}; account.provider_groups = {"paid"};
    account.keys = {{"one", "access-one", true}, {"two", "access-two", true}};
    account.requests_per_minute = 1;
    account.tokens_per_day = 128; account.token_reservation = 64;
    cfg.clients = {account};
    proxy.updateConfig(cfg);
    client.set_default_headers({{"Authorization", "Bearer access-one"}});
    first.set(); second.set({200, R"({"text":"hello","usage":{"input_tokens":0,"output_tokens":0}})"});
    auto denied_model = client.Post("/v1/images/generations", R"({"model":"dall-e-2","prompt":"hill"})", "application/json");
    LR_CHECK(denied_model && denied_model->status == 403);
    auto admitted_audio = client.Post("/v1/audio/transcriptions", upload, mime);
    LR_CHECK(admitted_audio && admitted_audio->status == 200);
    LR_CHECK_EQ(first.requests, 0); LR_CHECK_EQ(second.requests, 1);
    LR_CHECK_EQ(second.last().parts[1].content, "upstream-b");
    const auto account_usage = [&]() {
        auto snapshot = proxy.snapshot();
        const auto found = std::ranges::find_if(snapshot.clients, [&](const auto &usage) { return usage.client == cfg.clients[0].id; });
        LR_CHECK(found != snapshot.clients.end());
        return found == snapshot.clients.end() ? ClientUsage{} : *found;
    };
    LR_CHECK_EQ(account_usage().tokens_today, 0); // explicit zero releases its reservation
    client.set_default_headers({{"Authorization", "Bearer access-two"}});
    auto rate_limited = client.Post("/v1/images/generations", R"({"model":"media","prompt":"hill"})", "application/json");
    LR_CHECK(rate_limited && rate_limited->status == 429);
    LR_CHECK_EQ(second.requests, 1);
    auto no_admin = client.Get("/__literouter/config");
    LR_CHECK(no_admin && no_admin->status == 401);

    LR_GROUP("explicit zero SSE usage frees token quota while missing audio usage keeps its reserve");
    cfg.clients[0].id = "zero-stream";
    cfg.clients[0].requests_per_minute = 0;
    cfg.clients[0].tokens_per_day = 64;
    proxy.updateConfig(cfg);
    second.set({200, "data: {\"usage\":{\"input_tokens\":0,\"output_tokens\":0}}\n\n", "text/event-stream", true});
    for (int i = 0; i < 2; ++i) {
        auto zero = client.Post("/v1/images/generations", R"({"model":"media","prompt":"hill","stream":true})", "application/json");
        LR_CHECK(zero && zero->status == 200);
        LR_CHECK(settled(proxy));
        LR_CHECK_EQ(account_usage().tokens_today, 0);
    }
    cfg.clients[0].id = "error-usage";
    cfg.clients[0].tokens_per_day = 128;
    proxy.updateConfig(cfg);
    second.set({400, R"({"error":{"message":"rejected"},"usage":{"input_tokens":3,"output_tokens":7}})"});
    auto rejected_with_usage = client.Post("/v1/audio/speech", R"({"model":"media","input":"hi","voice":"alloy"})", "application/json");
    LR_CHECK(rejected_with_usage && rejected_with_usage->status == 400);
    LR_CHECK_EQ(account_usage().tokens_today, 10);
    cfg.clients[0].id = "unknown-audio";
    cfg.clients[0].tokens_per_day = 64;
    proxy.updateConfig(cfg);
    second.set({200, binary, "audio/wav", true});
    auto unknown_usage = client.Post("/v1/audio/speech", R"({"model":"media","input":"hi","voice":"alloy"})", "application/json");
    LR_CHECK(unknown_usage && unknown_usage->status == 200);
    LR_CHECK(settled(proxy));
    LR_CHECK_EQ(account_usage().tokens_today, 64);
    LR_CHECK_EQ(account_usage().reserved_tokens, 0);
    auto token_limited = client.Post("/v1/audio/transcriptions", upload, mime);
    LR_CHECK(token_limited && token_limited->status == 429);

    LR_GROUP("a canceled media stream releases the account permit exactly once");
    cfg.clients[0].id = "cancel-stream";
    cfg.clients[0].tokens_per_day = 256;
    cfg.clients[0].max_concurrent = 1;
    proxy.updateConfig(cfg);
    second.set({200, binary, "audio/wav", true, false, true});
    std::atomic<bool> cancelled{false};
    std::thread aborting([&] {
        httplib::Client connection{proxy.boundAddress(), proxy.boundPort()};
        connection.set_read_timeout(5, 0);
        httplib::Request req;
        req.method = "POST"; req.path = "/v1/audio/speech";
        req.set_header("Authorization", "Bearer access-one");
        req.set_header("Content-Type", "application/json");
        req.body = R"({"model":"media","input":"hi","voice":"alloy"})";
        req.content_receiver = [](const char *, std::size_t, std::size_t, std::size_t) { return false; };
        cancelled = !connection.send(req);
    });
    const auto wait_limit = std::chrono::steady_clock::now() + std::chrono::seconds(4);
    while (!second.streaming_entered && std::chrono::steady_clock::now() < wait_limit)
        std::this_thread::sleep_for(std::chrono::milliseconds(5));
    LR_CHECK(second.streaming_entered.load());
    auto concurrent = client.Post("/v1/audio/transcriptions", upload, mime);
    LR_CHECK(concurrent && concurrent->status == 429);
    second.release_stream = true;
    aborting.join();
    LR_CHECK(cancelled.load()); LR_CHECK(settled(proxy));
    LR_CHECK_EQ(account_usage().active_requests, 0);
    LR_CHECK_EQ(account_usage().requests, 1);
    LR_CHECK_EQ(account_usage().successes + account_usage().failures, 1);
    LR_CHECK_EQ(account_usage().tokens_today, 64);
    second.set({200, R"({"usage":{"input_tokens":0,"output_tokens":0}})"});
    auto after_abort = client.Post("/v1/audio/transcriptions", upload, mime);
    LR_CHECK(after_abort && after_abort->status == 200);

    LR_GROUP("exhausted transports keep unknown token charges but empty authorized routes refund them");
    cfg.clients[0].id = "exhausted";
    cfg.clients[0].tokens_per_day = 64;
    httplib::Server closed;
    const int closed_port = closed.bind_to_any_port("127.0.0.1"); closed.stop();
    cfg.providers[1].base_url = std::format("http://127.0.0.1:{}/api/v1", closed_port);
    proxy.updateConfig(cfg);
    auto exhausted = client.Post("/v1/audio/speech", R"({"model":"media","input":"hi","voice":"alloy"})", "application/json");
    LR_CHECK(exhausted && exhausted->status >= 500);
    LR_CHECK(settled(proxy));
    LR_CHECK_EQ(account_usage().tokens_today, 64);
    auto exhausted_again = client.Post("/v1/audio/speech", R"({"model":"media","input":"hi","voice":"alloy"})", "application/json");
    LR_CHECK(exhausted_again && exhausted_again->status == 429);
    cfg.clients[0].id = "no-channel";
    cfg.clients[0].provider_groups = {"unavailable"};
    proxy.updateConfig(cfg);
    auto no_channel = client.Post("/v1/audio/transcriptions", upload, mime);
    LR_CHECK(no_channel && no_channel->status == 404);
    LR_CHECK_EQ(account_usage().tokens_today, 0);
    proxy.stop();
